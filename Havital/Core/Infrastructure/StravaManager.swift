...
class StravaManager: NSObject, ObservableObject {
    ...
    func checkConnectionStatus() async {
        ...
        await TrackedTask("StravaManager: checkConnectionStatus") {
            ...
            // 如果需要恢復數據源，在 MainActor.run 之外進行異步操作 (Clean Architecture: Manager → Repository)
            if shouldRestoreDataSource {
                do {
                    try await self.userProfileRepository.updateDataSource(DataSourceType.strava.rawValue)
    ...
    func handleCallback(url: URL) async {
        ...
        if success == "true" {
            ...
            // ✅ 改為阻塞式更新，確保後端更新完成再返回 (Clean Architecture: Manager → Repository)
            do {
                try await userProfileRepository.updateDataSource(DataSourceType.strava.rawValue)
