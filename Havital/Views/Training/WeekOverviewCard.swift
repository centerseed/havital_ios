...
struct WeekOverviewCard: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showWeekSelector) {
            NavigationView {
                List(viewModel.availableWeeks.reversed(), id: \.self) { week in
                    HStack {
                        Text("第 \(week) 週")
                        Spacer()
                        Button("查看課表") {
                            let planId = "\(viewModel.trainingOverview?.id ?? "?")_\(week)"
                            print("[UI] selectedWeek=\(week), planId=\(planId)")
                            Task {
                                await viewModel.fetchWeekPlan(week: week, healthKitManager: healthKitManager)
                            }
