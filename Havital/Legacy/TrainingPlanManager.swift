...
///
/// ⚠️ DEPRECATED: 此類已棄用，請使用 Clean Architecture 組件:
/// - 數據存取: TrainingPlanRepository (Features/TrainingPlan/Domain/Repositories/)
/// - UI 狀態管理: TrainingPlanViewModel (Features/TrainingPlan/Presentation/ViewModels/)
/// - 本地緩存: TrainingPlanLocalDataSource (Features/TrainingPlan/Data/DataSources/)
///
/// 遷移指南:
/// 1. ViewModel 層使用 TrainingPlanRepository 替代 TrainingPlanManager.shared
/// 2. 緩存檢查使用 TrainingPlanLocalDataSource.getOverview()
/// 3. 事件監聽使用 CacheEventBus 訂閱機制
@available(*, deprecated, message: "Use TrainingPlanRepository and TrainingPlanViewModel instead")
class TrainingPlanManager: ObservableObject, DataManageable {
    
    // MARK: - Type Definitions
    typealias DataType = WeeklyPlan
    typealias ServiceType = TrainingPlanRepository
    ...
    // MARK: - Dependencies
    let service: TrainingPlanRepository
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    if self.currentWeeklyPlan == nil {
                        ...
                        } else if error is TrainingPlanError {
    ...
    private func performLoadWeeklyPlan() async throws {
        ...
        do {
            ...
            let overview = try await service.getOverview()
    ...
    private func fetchLatestWeeklyPlan() async throws -> WeeklyPlan? {
        ...
        do {
            return try await service.getWeeklyPlan(planId: planId)
        } catch TrainingPlanError.weeklyPlanNotFound {
            // 404 是正常情況，表示該週還沒有計劃
            return nil
        }
    }
        ...
        await executeTask(id: TaskID("load_training_overview")) { [weak self] () -> Void in
            ...
            do {
                let overview = try await APICallTracker.$currentSource.withValue("TrainingPlanManager: loadTrainingOverview") {
                    try await self.service.getOverview()
                }
    ...
    func generateNewWeekPlan() async -> Bool {
        return await executeTask(id: TaskID("generate_new_week_\(selectedWeek)")) { [weak self] () -> Void in
            ...
            do {
                ...
                // 使用實際的 API 方法
                let newPlan = try await self.service.createWeeklyPlan(week: self.selectedWeek, startFromStage: nil, isBeginner: false)
