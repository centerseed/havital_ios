...
class VDOTStorage {
    ...
    func saveVDOTData(points: [VDOTDataPoint], needUpdatedHrRange: Bool) {
        do {
            // 保存資料點
            let encodablePoints = points.map { EncodableVDOTDataPoint(date: $0.date.timeIntervalSince1970, value: $0.value) }
    ...
    func loadVDOTData() -> ([VDOTDataPoint], Bool) {
        ...
        do {
            ...
            let vdotPoints = decodedPoints.map {
                VDOTDataPoint(date: Date(timeIntervalSince1970: $0.date), value: $0.value)
            }
    ...
    func shouldRefreshData(cacheTimeInMinutes: Double = 10) -> Bool {
        ...
        let timeSinceLastFetch = currentTime.timeIntervalSince(lastFetchTime) / 60 // 轉換為分鐘
        
        // 如果距離上次獲取時間超過指定分鐘數，應該刷新
        return timeSinceLastFetch > cacheTimeInMinutes
    }
...
// 用於編碼/解碼的輔助結構
fileprivate struct EncodableVDOTDataPoint: Codable {
