...
### AC-AUTH-02: Login 畫面提供目前支援的三種登入入口

Given 使用者位於 `LoginView`，  
When 畫面載入完成，  
Then 系統必須提供 Google Sign-In、Apple Sign-In、Demo Login 三個入口，並在請求進行中顯示 loading、禁止重複點擊。
...
- Demo Login 雖無 Firebase session，仍必須滿足與正式登入相同的入口切換結果
