...
**Then** 顯示 4 個 group：AI 個人化週課表 / 隨時調整訓練 / AI 週回顧 / 賽事目標規劃，每組 title 與 bullets 從 `paywall.premium.features.{group}.*` 對應 i18n key 渲染（group keys = `plan` / `adjust` / `review` / `race`）。
**And** Features 列舉的能力必須與 Gating 範圍對齊——不出現未實作功能（如 Rizo）或非 gated 功能（如 race prediction、advanced analytics）。

#### AC-PAYWALL-11: Features 文案不含品牌相對訴求
**Given** Features 區塊
**When** QA review 任何語言版本的 bullet 文案
**Then** 不出現「unlock your potential」「升級以解鎖完整功能」等抽象空話；每條 bullet 描述具體能力（如「每週自動生成的智能課表」）。
