...
此 spec 原始內容以需求段落撰寫；為了讓後續派工、review 與 QA 可以穩定引用，以下 AC ID 視為本 spec 的正式索引。

| AC ID | 對應需求 |
|------|----------|
| AC-ONB-01a | P0-1 所有頁面 CTA 位置 / 大小 / 圓角 / 間距一致 |
| AC-ONB-01b | P0-1 CTA 固定在底部，不隨內容推出螢幕 |
| AC-ONB-01c | P0-1 標題字型 / 水平間距 / CTA 樣式一致 |
| AC-ONB-02a | P0-2 Intro CTA 始終可見 |
| AC-ONB-02b | P0-2 不同裝置尺寸下 CTA 不超出可見區 |
| AC-ONB-03a | P0-3 每頁只有一個主要下一步入口 |
| AC-ONB-03b | P0-3 WeeklyDistance 的 skip 為次要文字入口 |
| AC-ONB-04a | P0-4 用戶可快速精確設定週跑量 |
| AC-ONB-04b | P0-4 每次操作步進不超過 5km |
| AC-ONB-05a | P0-5 距離 / 時間 picker 不超過畫面 60% |
| AC-ONB-05b | P0-5 編輯器可一鍵確認退出 |
| AC-ONB-06a | P0-6 非首頁步驟都有一致返回方式 |
| AC-ONB-06b | P0-6 DataSourceSelectionView 也有可見返回方式 |
| AC-ONB-07 | P0-7 DataSourceSelectionView 返回後狀態正確 |
| AC-ONB-08a | P0-8 英文模式無硬編碼中文殘留 |
| AC-ONB-08b | P0-8 繁中模式正確顯示在地化文字 |
| AC-ONB-09a | P0-9 PB 跨步驟返回後仍保留 |
| AC-ONB-09b | P0-9 目標類型可被後續步驟正確讀取 |
| AC-ONB-09c | P0-9 已取得資料不得重複 API 請求 |
| AC-ONB-10a | P0-10 賽事目標頁提供資料庫選擇 / 手動輸入雙入口 |
| AC-ONB-10b | P0-10 列表載入後顯示賽事核心資訊 |
| AC-ONB-10c | P0-10 關鍵字搜尋可即時過濾 |
| AC-ONB-10d | P0-10 距離篩選可收斂賽事列表 |
| AC-ONB-10e | P0-10 地區切換為顯式控制，不依賴裝置語系 |
| AC-ONB-10f | P0-10 多距離賽事以 sheet / dialog 選擇 |
| AC-ONB-10g | P0-10 選定賽事後自動回填名稱 / 距離 / 日期 |
| AC-ONB-10h | P0-10 選定後顯示倒數天數 |
| AC-ONB-10i | P0-10 API 失敗時回退手動輸入，不阻斷 onboarding |
| AC-ONB-10j | P0-10 re-onboarding 可沿用同一賽事選擇能力 |
| AC-ONB-10k | P0-10 過期賽事隱藏、截止報名但未開賽賽事仍可顯示 |
| AC-ONB-11a | P1-1 顯示目前進度 |
| AC-ONB-11b | P1-1 分支路徑反映實際比例 |
| AC-ONB-11c | P1-1 進度只進不退 |
| AC-ONB-12a | P1-2 disabled / enabled CTA 視覺差異明顯 |
| AC-ONB-12b | P1-2 enabled 時有視覺反饋 |
| AC-ONB-13a | P1-3 onboarding 心率頁提供預設值 |
| AC-ONB-13b | P1-3 預設值說明「先用後補」 |
| AC-ONB-13c | P1-3 使用預設值也必須正常儲存並參與計算 |
| AC-ONB-13d | P1-3 手動修改後不再顯示預設值提示 |
| AC-ONB-13e | P1-3 後續 profile / settings 有更新引導 |
| AC-ONB-14a | P1-4 長列表捲動流暢 |
| AC-ONB-14b | P1-4 搜尋延遲在可接受範圍內 |
| AC-ONB-14c | P1-4 賽事列表預設按日期排序 |
