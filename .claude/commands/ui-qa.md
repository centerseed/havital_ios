...
按以下順序操作（用座標表直接 tap，只在迷路時 `ui_describe_all`，不截圖）：
...
### 🔴 必須遵守
1. **只截 2 張圖**（開始 + 結束）— 中間用 `ui_describe_point` 驗證，不截圖
2. **已知座標直接 tap** — 本文件座標表已實測，不需先 `ui_describe_all`。只在座標失效或迷路時才用 `ui_describe_all`（每畫面最多 1 次）
3. **連續操作不等待** — `tap → sleep 1 → tap → sleep 1 → describe_point 驗證`，不要每步都停下分析
4. **等待載入** — 頁面切換等 1-2 秒，API 呼叫（產生課表）等 5-10 秒
5. **處理系統彈窗** — 啟動後先檢查
6. **失敗最多重試 2 次** — 仍失敗就記錄問題，不硬過
7. **報告必須產出** — 每個測試案例都要有報告檔案
